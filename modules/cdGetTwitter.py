...
def getDates(uri, date_from, date_until, verbose=False):
    ...
    # [request url]%20since%3A[YYYY-MM-DD]%20until%3A[YYYY-MM-DD]&src=typd

    from_date_str = date_from.strftime("%Y-%m-%d")
    until_date_str = date_until.strftime("%Y-%m-%d")
    search_str = ('https://twitter.com/search?f=tweets&q={uri}'
                  '%20since%3A{from_date}%20until%3A{until_date}'
                  '&src=typd').format(
        uri=uri, from_date=from_date_str, until_date=until_date_str
    )

    logging.debug(search_str)
    logging.debug('getTwitter: Search: from %s to %s' %
    ...
    # get all tweets text in result this is used for get amount for results
    ...
    for i in range(0, len(tweet_text)):
        ...
        time_stamp = datetime.datetime.fromtimestamp(
            int(tweet_timestamps[i].find('span', 'js-short-timestamp')
                ['data-time']))
...
def getEarliestDate(uri, from_date, until_date, verbose=False):
    '''
    Get most likely oldest date of tweet that has a uri ina given time period
    with a binar search
    '''
    ...
    if len(result) == 0:
        return None
    # if there are result (less than 10 or search bound closed to one day),
    # return last one (oldest one)
    if len(result) <= 10 or until_date - from_date \
...
def getTwitter(uri, outputArray, indexOfOutputArray, verbose=False, **kwargs):
    ...
    date_str = ''
    date = getEarliestDate(converted_url, earliest_time,
                           datetime.datetime.now(), verbose)
    if date is not None:
    ...
    if uri.startswith('www.'):
        ...
        logging.debug("getTwitter: Trying %s", converted_url)
        date2 = getEarliestDate(
            converted_url, earliest_time, datetime.datetime.now(), verbose)
